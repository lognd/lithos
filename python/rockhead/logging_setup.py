...
``rockhead._core`` logger hierarchy, so they render identically to Python
records. Root level is read from ``ROCKHEAD_LOG`` (default ``INFO``).
...
def _config() -> dict[str, object]:
    ...
    level = os.environ.get("ROCKHEAD_LOG", "INFO").upper()
    return {
        "version": 1,
