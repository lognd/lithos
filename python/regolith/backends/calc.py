...
def _build_sheet(
    ...
    *,
    ...
    # directly against a unit-bearing literal -- which now includes
    # every SI claim (`elec.impedance`/`elec.termination`) too, since
    # D256's Rust root fix preserves the unit token on `rhs` directly
    # instead of discarding it. The WO-128/F144 closed-SI-vocabulary
    # THIRD fallback this comment used to describe is deleted (D256.4):
    # it was a second unit channel duplicating this one, tolerated for
    # exactly one cycle.
