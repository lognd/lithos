...
# frob:tests python/regolith/harness/models/timing.py::stackup_v_p_mm_per_ns kind="unit"
def test_v_p_matches_hand_computed_tem_relation() -> None:
...
def test_v_p_rejects_nonpositive_dk() -> None:
    with pytest.raises(ValueError, match="positive"):
        stackup_v_p_mm_per_ns(0.0)


# frob:tests python/regolith/harness/models/timing.py::route_delay_ns kind="unit"
def test_route_contribution_pessimal_ns_matches_hand_computed_delay() -> None:
...
def test_bare_literal_contribution_is_unrepresentable() -> None:
...
def test_both_grounding_sources_at_once_is_refused() -> None:
    with pytest.raises(ValidationError, match="BOTH"):
        TimingContribution(
            name="over_specified",
            cited=_cited_interval(5.0),
            route_length_mm=10.0,
            route_dk=Cited(value=4.0, citation=_synthetic_citation(), confirmed=True),
        )


def test_route_missing_dk_is_refused() -> None:
...
def test_closes_over_cited_datasheet_values_within_limit() -> None:
    """t_pd + t_su under a 20ns clock period: closes with slack."""
    t_pd = TimingContribution(name="t_pd", cited=_cited_interval(6.0))
    t_su = TimingContribution(name="t_su", cited=_cited_interval(4.0, table="table 3"))
...
def test_e0432_negative_fixture_names_the_right_worst_contributor() -> None:
...
def test_symbolic_pessimal_bound_is_a_domain_error_not_a_crash() -> None:
...
def test_empty_contributions_is_a_domain_error() -> None:
    result = close_timing_budget("empty_budget", 10.0, [])
    assert result.is_err, result


# frob:tests python/regolith/harness/models/timing.py::TimingBudgetModel.output_unit kind="unit"
...
# frob:tests python/regolith/harness/models/timing.py::TimingContribution.citation_summary kind="unit"
# frob:tests python/regolith/harness/models/timing.py::timing_closure_given_loads kind="unit"
def test_timing_table_renders_with_citations_visible() -> None:
    ...
    t_pd = TimingContribution(name="t_pd", cited=_cited_interval(6.0, table="table 9"))
    t_su = TimingContribution(name="t_su", cited=_cited_interval(4.0, table="table 10"))
