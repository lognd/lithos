...
| `std.elec` | interfaces, matings, components | port/family/protocol/bus packs (buffers, buses, digital, families, logic, power, protocols, sense) PLUS (WO-66, D174/D179) a GENERATED `passive_family` E-series batch (`records/e_series.toml`, IEC 60063 resistor/capacitor parametric families) and a hand-cited `connectors.toml` batch (JST-XH/PH, Molex KK, screw-terminal classes). WO-66 follow-up batch 2 (2026-07-10) adds `records/cells.toml` (Samsung INR18650-25R cylindrical Li-ion class, EEMB LP503759 pouch LiPo class, plus a `series_pack` class deriving voltage/mass by declared cell x S-count arithmetic -- not a transcribed vendor pack SKU) and `records/servos.toml` (Hitec HS-311 standard + HS-55 micro RC servo classes), all hand-cited manufacturer datasheet figures. Cycle-33 F112 queue batch 2 (2026-07-10) adds two more `crystals.toml` rows (Epson FA-238 32.768kHz tuning-fork RTC crystal, Abracon AB-8.000MHZ HC-49/US class) and three more `connectors.toml` rows (JST-PH 4-position, Molex-KK 6-position, and a 0.1in board-to-board pin-header class carrying the first `exposure_class = "internal"` example), same hand-cited shape as the WO-79/WO-66 batches. Config-strap rows (the third WO-79-pattern item on the same queue entry) were evaluated and CUT: no stdlib package hosts MCU strap records today (the three straps: tables that exist -- `examples/registry/{rp2040,atsamd21,stm32g0}.cupr` -- are corpus, not stdlib/, out of this batch's scope), and no additional MCU part could be cited to the same section-number precision those three already meet without risking the fabricated-precision failure mode `rp2040.cupr`'s own header explicitly warns against; named growth, not silently dropped. |
...
| `std.fluid` | materials | media property tables + fitting/loss/pipe-schedule records; WO-60 (D166) added a `components.toml` batch (pump, ball valve, in-line filter, metering orifice), real-catalog cited; WO-64 W1/W2 (WO-66 follow-up, D182) added `records/polymer_melt.toml` (PLA/PETG melt-medium classes: solid density + cited melt-density/zero-shear-viscosity point sets, PETG's melt density/viscosity OMITTED with a note pending a verifiable source) and two more `[[pump]]` rows in `components.toml` (Sunon 4020 12V DC axial fan, Delta 5015 24V DC radial blower -- rated flow endpoints only, real-catalog cited) |
