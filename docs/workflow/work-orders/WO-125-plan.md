...
- [x] rename ship `--profile` -> `--emit-profile` (help text + error
...
- [x] `stdlib/std.elec/records/dft.toml` gains one
...
- [x] `TapHeaderRecord` model + `load_tap_header_record()` loader in
...
- [x] `tap_candidates_from_payload(payload)` -- claim-named nets/
...
- [x] `explicit_taps_from_debug_spec` / `hdl_debug_pins_from_debug_spec`
      -- the ship spec's `"debug"` block (WO-102 spec-block idiom):
      `{"debug": {"taps": [...], "hdl_debug_pins": {"<subj>": [...]}}}`.
- [x] explicit-tap resolution: exact `target_path` match, else UNIQUE
...
- [x] `regolith/realizer/elec/debug_placement.py`: `derive_tap_placements
      ...
      D224).
- [x] WO-124 is in flight in parallel and its silkscreen seam is NOT
...
- [x] `ElecBackend` serializes `tap_placements.json` for its subject
      when the plan is present (backends serialize, never decide).

D5 -- firmware augmentation:
- [x] `FirmwareBackend` emits `generated/debug_taps.h` per subject
...
- [x] a design with no firmware subjects: named absence row in the
      tap map (never a fabricated table).

D6 -- HDL augmentation:
- [x] `HdlBackend` emits `src/debug_taps.v` per subject when taps
...
- [x] ship emits `harness/tap_map.json` (charter 40 sec. 3's family;
...
- [x] `check_tap_agreement(tap_map_bytes, files)` re-parses the
      EMITTED bytes (markers), both directions; failure = named
      diagnostic, ship refuses.
- [x] `docs/spec/regolith/13-invariants.md` INV-32 entry WITH proof
      argument, SAME change as the check.

Wiring (ship path):
- [x] `BackendInputs` gains `debug_taps`/`tap_header`/`tap_placements`/
      `hdl_debug_pins` (plain Python inputs class -- not wire schema).
- [x] `ship(..., debug_spec=None)`: profile=="debug" derives the tap
...
- [x] CLI ship passes `spec_data.get("debug")`.
- [x] `package.py` FAMILY_DIRS += "harness" (index line; release
      goldens regenerated + diff reviewed).

Acceptance projects:
- [x] mainboard_mx ship.spec.json gains a `"debug"` block (explicit
      tap on the claim-named refclk net); debug ship emits placed
      header + labeled test points + tap map passing INV-32.
- [x] riscv_hart_rv1 ship.spec.json gains `"debug"` with
...
Tests:
- [x] unit: header-record loader, candidate extraction, suffix
      resolution, placement plan determinism, generated .h/.v
      content, `check_tap_agreement` both failure directions.
- [x] integration: synthetic ship debug vs release -- release file
...
- [x] end-to-end CLI: mainboard_mx + riscv_hart_rv1 build --release
      then ship --emit-profile debug; assert artifacts + INV-32 +
      census equality vs the release ship of the same build.
- [x] goldens: `make demos` regenerated where the index family line
      lands; diff reviewed (no new error-level diagnostic rows).

Docs:
- [x] charter 40 cross-ref note (record home + emitted paths);
