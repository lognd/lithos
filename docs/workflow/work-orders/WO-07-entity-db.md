...
Status: done
Depends: WO-05, WO-06
Language: Rust (`regolith-sem`) -- see `../../spec/toolchain/00-architecture.md` (normative; supersedes Python-specific implementation notes below)
Spec: regolith/05 sec. 1, 3, 5; hematite/02 sec. 4; regolith/06
...
- `Region` entities with exclusion/arbitration policy (regolith/05
  regions rule).
...
- Scenario tests: pattern commit produces one orbit; off-pattern
  feature splits it; `any` legality queries answer per regolith/05
