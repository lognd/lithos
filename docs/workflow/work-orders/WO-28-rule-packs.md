...
`../../spec/toolchain/21-rule-packs.md` (normative for this WO), AD-21
...
2. Grammar: typed `RuleDecl`/`RulePackBlock`/`ExpectBlock` CST under
   `process` decls (`regolith-syntax`); `../../spec/toolchain/grammar.ebnf` in lockstep;
...
E0601-E0604 numbering from `../../spec/toolchain/21-rule-packs.md` sec. 3) and a real
...
1. **Static-rule evaluation, `resolves:` resolution, realized-fact
   ...
   ambiguity escalates to `../../spec/toolchain/00-architecture.md`/upstream WOs, it is not
...
`../../spec/toolchain/23-lowering-output-surface.md`) -- its design pass (cycle 19, D88)
