...
be E0420; see `examples/tracks/cuprite/fpga_mcu_board.cupr`.)
...
`examples/tracks/cuprite/sampled_buck.cupr` (continuous plant + sampled control
loop):
...
- **`by circuit`** realizes a block at component level (the analog
  ...
  `examples/tracks/cuprite/buck_converter.cupr`]: circuit bodies instantiate
