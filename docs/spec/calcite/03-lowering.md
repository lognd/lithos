...
| `civil.bearing_pressure(footing)` | frame ref (reactions) + soil record ref; closed-form model landed cycle 33 (D196): the footing's resolved gravity-path reaction (the D194 reaction machinery, generalized to any transfer target) over its declared bearing area vs the soil allowable -- the two remaining input gaps defer by NAME (`unresolved_limit`: the `site.soil.bearing` comparator is not literalized at lowering, embedment-only today; `footing_area_undeclared`: `std.civil`'s `BasePlate` class carries no bearing-area parameter yet) |
