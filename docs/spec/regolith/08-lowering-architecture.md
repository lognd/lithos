...
**L4 representations are concrete IRs** (landed cycle 24-25, D128/
AD-25, machinery WO-42): realized-domain content -- mech realized
geometry, elec placed/routed layout, and every future sibling --
crosses to pipeline passes, rule packs, and solver packs only as
...
(`regolith.orchestrator.orchestrate.staged_build`).
...
| L4 (fluid, WO-32) | none -- flownet parameters extract from the OWNING part's realized mech geometry, never written directly | none: fluorite has no `by extern` form of its own; a fluid edge's realized wall data arrives through the same mech `import`/kernel path any other part does | extraction is in-pipeline (D128/AD-25 above) over the `flownet` content-addressed payload, cited to the geometry snapshot hash; a transient/volume-budget claim over an edge with neither a compliance record nor an extractable wall is a compile diagnostic (E0203, fluorite/03 sec. 1), not a T2-measurement gap |
