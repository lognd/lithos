...
| `22-proving-optimizations.md` | the `demos/` proof packs: `make demos`, physical artifacts (drawings/STEP/GLB/BOM) per optimizer surface AND per user-facing feature family (WO-108/WO-115, D218/D222), the content-hashed manifest + PROOF.md, and the completeness/determinism gate |
...
| `31-diagnostics-and-explain.md` | the ONE code space (D247/WO-131): the families incl. E09xx/E10xx/E11xx, the code-stability rule, `regolith explain <code> [--json]`, the machine-checked completeness legs, and how a producer adds a new coded failure |
