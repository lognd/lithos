...
//!
//! The encoder itself lives in `rockhead_util::canon` (AD-18) -- the
//! bottom of the layering, shared by `rockhead-sem` snapshot hashes and
//! `rockhead-oblig` obligation keys. This module re-exports it
//! unchanged so no downstream caller (`rockhead-api`, `rockhead-py`,
//! the Python facade, `make schema`) sees a path change, and owns the
//! schemars export, which stays in `rockhead-oblig` per AD-5.

pub use rockhead_util::canon::{canonical_cbor, content_address, EncodeError};
