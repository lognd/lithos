...
//! `catch_unwind`-wrapped so a Rust panic becomes a `rockhead.CoreBug`
...
impl PyCoreSession {
    ...
    fn new(paths: Vec<String>) -> PyResult<Self> {
        guard(|| {
            ...
            PyCoreSession {
                inner: rockhead_api::Session::open_files(files),
            }
        })
    }
...
fn debug_dump(stage: &str, path: &str) -> PyResult<String> {
    ...
    match guard(|| rockhead_api::debug_dump(stage, &path))? {
        Ok(text) => Ok(text),
        Err(e) => Err(core_error(&e)),
    }
}
