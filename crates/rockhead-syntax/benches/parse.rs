...
fn bench_frontend(c: &mut Criterion) {
    ...
    for (name, source) in &sources {
        ...
        group.bench_with_input(BenchmarkId::new("lex", name), source, |b, src| {
            b.iter(|| rockhead_syntax::token::lex(std::hint::black_box(src)));
        });
        group.bench_with_input(BenchmarkId::new("parse", name), source, |b, src| {
            b.iter(|| rockhead_syntax::parser::parse(std::hint::black_box(src), &file));
        });
        group.bench_with_input(BenchmarkId::new("format", name), source, |b, src| {
            b.iter(|| rockhead_syntax::formatter::format(std::hint::black_box(src), &file));
        });
    }
    group.finish();
}
