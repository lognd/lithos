...
//! formats: `.hem` (hematite) and `.cupr` (cuprite).
...
pub const EXTENSIONS: &[(&str, Language)] =
    &[("hem", Language::Hematite), ("cupr", Language::Cuprite)];

/// Resolve a bare extension (no leading dot, case-sensitive) to its
/// language, or `None` if it is not a rockhead source file.
#[must_use]
...
mod tests {
    ...
    fn recognizes_settled_extensions() {
        assert_eq!(language_for_extension("hem"), Some(Language::Hematite));
