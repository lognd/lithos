...
| `negative/db_illegal.cupr` | DELIBERATE NEGATIVE | `30dBm + 27dBm` dies at L1 with E0104 (regolith 02 sec. 5a); excluded from corpus |

## Candidate findings (D119 ledger -- promotion is the coordinator's)

1. **Negative-literal log sums escape E0104.** `30dBm + -110dBm >=
   12dBm` passes `check` today: the unary minus makes the right leaf
   a non-log expression, so the log-term flattening in
   `crates/regolith-syntax/src/checks.rs` (`log_terms`) bails out and
   the L1 legality check never fires. Negative referenced literals
   are the COMMON case in link budgets (sensitivities live around
   -110dBm), so the classic bug the check exists for slips through in
   its most likely spelling. Both spellings are on record in
   `negative/db_illegal.cupr`. Candidate fix: fold unary minus on a
   log-unit literal into the signed term list.
